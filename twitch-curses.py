...
query_limit = "75"
api_key = "Client-ID:caozjg12y6hjop39wx996mxn585yqyk"
highlight = 0
page = 0
hl_cache = 0
p_cache = 0
state = "top"
q = ["1080p60", "1080p", "720p60", "720p", "480p", "360p", "160p", "audio_only"]
quality = 3
key = 0
donothing = False
...
def start_player(url, quality):
	return subprocess.call(["mpv", "--http-header-fields=" + api_key, url, "--ytdl-format=" + quality])

def init_player(url):
	print("[twitch-curses] Launching mpv")
	ls_exit_code = start_player(url, q[quality])
	while ls_exit_code != 0:
		subprocess.call(["youtube-dl", "-F", url])
		print("\n[twitch-curses] Mpv returned an error. This usually means that the selected stream quality is not available. If that is the case, then you can now choose one of the available streams printed below (defaults to 'best' if left empty). Or you can type 'A' to abort.")
		selected_stream = input("Stream to open [best]: ")
		if selected_stream == "A" or selected_stream == "a":
			break
		if selected_stream == "":
			selected_stream = "best"
		ls_exit_code = start_player(url, selected_stream)
...
try:
	...
	while key != ord('q') and key != ord('Q'):
		if windowsize[0] < 10 or windowsize[1] < 32:
			...
			win_r.addstr(windowsize[0]-5, windowsize[1]//2-8, "url: u")
		...
		if key == curses.KEY_DOWN or key == ord('j'):
			...
			if state == "search":
				...
				init_player(currentpage[highlight]['channel']['url'])
				stdscr = curses.initscr()
				curses.noecho()
				curses.cbreak()
				stdscr.keypad(1)
			...
			if state != "top":
				...
				highlight = hl_cache
				page = p_cache
		elif (key == ord('+') or key == ord('=')) and quality:
			quality -= 1
		elif (key == ord('-') or key == ord('_')) and quality < len(q) - 1:
			quality += 1
			...
			highlight = 0
			page = 0
		elif key == ord('u') or key == ord('S'):
			urlbox = curses.newwin(3, windowsize[1]-4, windowsize[0]//2-1, 2)
			urlbox.border(0)
			urlbox.addnstr(0, 3, "Open url:", windowsize[0]-4)
			urlbox.refresh()
			curses.echo()
			s = urlbox.getstr(1,1, windowsize[1]-6)
			windowsize = init_display(stdscr)
			curses.endwin()
			init_player(currentpage[highlight]['channel']['url'])
			curses.noecho()
